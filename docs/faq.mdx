...
2. **Wrong namespace**: Ensure you're using the same `indexName`
...
```typescript
import { createContextWindow, ContextWindow, AskResult } from "context-window";

const cw: ContextWindow = await createContextWindow({ /* ... */ });
