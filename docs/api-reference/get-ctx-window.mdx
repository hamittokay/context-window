...
```typescript
function getCtxWindow(indexName: string): ContextWindow
```

## Parameters

<ParamField path="indexName" type="string" required>
  The unique identifier of the context window to retrieve. Must match the `indexName` used when creating the context window with `createCtxWindow()`.
</ParamField>
...
<Warning>
  Throws an error if no context window with the specified `indexName` exists in the registry.
</Warning>
...
function safeGetCtxWindow(indexName: string) {
  if (!hasCtxWindow(indexName)) {
    throw new Error(`Context window '${indexName}' has not been initialized`);
  }
  return getCtxWindow(indexName);
}
...
async function getOrCreateContextWindow(indexName: string, dataPath: string) {
  if (!hasCtxWindow(indexName)) {
    console.log(`Initializing ${indexName}...`);
    await createCtxWindow({
      indexName,
      data: [dataPath],
      ai: { provider: "openai" },
...
function debugGetCtxWindow(indexName: string) {
  console.log(`Retrieving context window: ${indexName}`);
  const cw = getCtxWindow(indexName);
  console.log(`Successfully retrieved: ${indexName}`);
  return cw;
}
```
