...
```typescript
function hasCtxWindow(indexName: string): boolean
```
...
```typescript
function deleteCtxWindow(indexName: string): boolean
```
...
class SessionManager {
  async createUserSession(userId: string, documents: string[]) {
    const indexName = `session-${userId}`;
    await createCtxWindow({
      indexName,
      data: documents,
      ai: { provider: "openai" },
...
class TenantManager {
  ...
  async initializeTenant(tenantId: string, documents: string[]) {
    const indexName = `${this.prefix}${tenantId}`;
